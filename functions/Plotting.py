...
def plot_PSD_Q1 (dt: float, P: np.ndarray, T1: np.ndarray, T: np.ndarray, t_start: float, t_end:float, omega):
    """ Takes an array of P , T1, and T over time and creates two subplots with their PSDs."""
    fs = 1/dt
    ...
    axs[0].plot(2*np.pi/omega*f_P, PSD_P)
    axs[0].set_ylabel('PSD of power')
    axs[0].set_xlabel(r"$\frac{2 \pi f}{\omega}$ [-]")
    axs[0].grid()
    #axs[0].set_title('PSD of Normal Load')
    
    axs[1].plot(2*np.pi/omega*f_T1, PSD_T1, 'tab:green', label = 'Thrust on 1 blade')
    axs[1].plot(2*np.pi/omega*f_T, PSD_T, 'tab:orange', label= 'Total thrust')
    axs[1].set_ylabel('PSD of thrusts')
    axs[1].set_xlabel(r"$\frac{2 \pi f}{\omega}$ [-]")
    axs[1].grid()
    axs[1].legend()
...
def plot_q4 (t_array: np.ndarray, dt: float, P: np.ndarray, pz: np.ndarray, radii: np.ndarray, r: float, T: np.ndarray, t_start: float, t_end:float, omega):
    """ Takes an array of P , T, and pz over time and creates six subplots with timeseries and relative PSDs."""
    fs = 1/dt
    r_idx = np.where(np.round(radii, 2)==r)
    pz_plot = np.squeeze(pz[0,:,r_idx])
    ...
    axs[0,0].plot(t_array, pz_plot)
    axs[0,0].set_ylabel('pz [N]')
    #axs[0,0].set_xlabel("Time [s]")
    axs[0,0].grid()

    axs[0,1].semilogy(2*np.pi/omega*f_pz, PSD_pz)
    axs[0,1].set_ylabel('PSD of pz')
    #axs[0,1].set_xlabel(r"$\frac{2 \pi f}{\omega}$ [-]")
    axs[0,1].grid()

    axs[1,0].plot(t_array, T/1e3)
    axs[1,0].set_ylabel('Thrust [KN]')
    #axs[1,0].set_xlabel("Time [s]")
    axs[1,0].grid()
    
    axs[1,1].semilogy(2*np.pi/omega*f_T, PSD_T)
    axs[1,1].set_ylabel('PSD of thrust')
    #axs[1,1].set_xlabel(r"$\frac{2 \pi f}{\omega}$ [-]")
    axs[1,1].grid()

    axs[2,0].plot(t_array, P/1e6)
    axs[2,0].set_ylabel('Power [MN]')
    axs[2,0].set_xlabel("Time [s]")
    axs[2,0].grid()

    axs[2,1].semilogy(2*np.pi/omega*f_P, PSD_P)
    axs[2,1].set_ylabel('PSD of power')
    axs[2,1].set_xlabel(r"$\frac{2 \pi f}{\omega}$ [-]")
    axs[2,1].grid()
